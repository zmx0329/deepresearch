...
RAG_EMBEDDING_PROVIDER = _get_env_choice(
    "RAG_EMBEDDING_PROVIDER", {"openai", "sentence_transformer"}, "openai"
)
RAG_SENTENCE_TRANSFORMER_MODEL = (
    os.getenv("RAG_SENTENCE_TRANSFORMER_MODEL", "BAAI/bge-small-zh-v1.5").strip()
    or "BAAI/bge-small-zh-v1.5"
)
RAG_SENTENCE_TRANSFORMER_DEVICE = (
    os.getenv("RAG_SENTENCE_TRANSFORMER_DEVICE", "cpu").strip() or "cpu"
)
...
LEARNING_REVIEW_MAX_OUTPUT_TOKENS = _require_positive(
    "LEARNING_REVIEW_MAX_OUTPUT_TOKENS", _get_env_int("LEARNING_REVIEW_MAX_OUTPUT_TOKENS", 4000) or 4000,
)
