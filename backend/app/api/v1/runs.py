...
async def export_report_html(run_id: str, database=Depends(get_database)):
    run = await require_run(database, run_id)
    message = await MessageRepository(database).get_assistant_for_run(run_id)
    if message is None or not message.content:
        raise AppError(ErrorCode.NOT_FOUND, "Run 报告不存在")
    return HTMLResponse(
        render_report_html(message.content, generated_at=run.completed_at),
        headers={
