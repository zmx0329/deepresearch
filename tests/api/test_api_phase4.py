...
def test_run_report_evidence_and_durable_sse_replay(client):
    ...
    html_report = client.get(f"/api/v1/runs/{run_id}/report.html")
    assert html_report.status_code == 200
    assert html_report.headers["content-type"].startswith("text/html")
    assert html_report.headers["content-disposition"] == f'attachment; filename="report-{run_id}.html"'
    assert "<h1" in html_report.text and "研究报告" in html_report.text
    assert 'class="evidence-ref"' in html_report.text
    assert 'data-ref="ev_' in html_report.text
    assert "<script" not in html_report.text.lower()
